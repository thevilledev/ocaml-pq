module Engine = Mlkem_engine.Mlkem512
include Engine
...
let require_random random length =
  ...
  if String.length value <> length then
    invalid_arg
      (Format.sprintf "Mlkem512: randomness callback returned %d bytes, expected %d"
...
let generate ~random () =
  ...
  | Error error -> invalid_arg (Format.asprintf "Mlkem512.generate: %a" pp_error error)
...
let encapsulate ~random ek =
  ...
  | Error error -> invalid_arg (Format.asprintf "Mlkem512.encapsulate: %a" pp_error error)
  | Ok result -> result
